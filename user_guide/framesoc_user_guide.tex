...
%% French title
\RRtitle{Framesoc User Guide}
%% English title
\RRetitle{Framesoc User Guide}
%%
...
%% TODO modify image: filter results
\begin{figure}[h!]
  \centering
...
Regarding timestamps representation in Framesoc, if no time unit is indicated beside a displayed timestamp, by default the time unit used is the one shown in the Trace Details~\ref{subsec:metadata} view for that trace (\texttt{Time-Unit} metadata). 
...
Right clicking on one or more of the table elements, a context menu is shown.
...
\begin{itemize}
 \item \emph{Exclude Item from statistics / Exclude items from statistics}: shown if only leaf items are selected. This command excludes the selected leaf items from statistics computation. 
 \item \emph{Merge Items}: shown only if two or more leaf items are selected. This command merges the items in a new folder item. The name and the color of the new folder item are chosen by the user with the dialog shown in Figure~\ref{fig:merge_dialog}.
    %% TODO better image please
    \begin{figure}[h!]
      \centering
	\includegraphics[width=.3\textwidth]{images/dialog_merge.png}
 ...
 \item \emph{Restore Excluded Items}: shown if at least on leaf item has been excluded from statistics computation. This command restores all previously excluded items into statistics computation.
\end{itemize}
...
Beside the three \emph{Show in...} buttons indicated as \num{13} (see Subsection~\ref{subsec:principles}), the view toolbar contains the following buttons:
 \begin{itemize}
  ...
  \item Button \num{8}: zoom.
  \item Button \num{9}: dezoom.
  \item Button \num{10}: display a dialog where it is possible to select the types to display.
  \item Button \num{11}: hide/display links.
...
For example, in Figure~\ref{fig:import_dialog} we can see that the \emph{Pajè Dump Importer} has a checkbox to enable double precision and a text field for passing trace files.
...
This dialog has the same structure of the Import Trace dialog (see Subsection~\ref{subsec:import}). 
...
The launch export tool dialog (Figure~\ref{fig:analysis_dialog}) allows the user to launch one of the export tools registered to the system.
This dialog has the same structure of the Import Trace dialog (see Subsection~\ref{subsec:import}). 
...
Its relative path to this directory is:  \texttt{./configuration/fr.inria.soctrace.lib.utils/soctrace.conf}.
